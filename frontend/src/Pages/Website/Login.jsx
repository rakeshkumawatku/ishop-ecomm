...
import { dbToSate } from '../../Reduser/cart';
...
const Login = () => {
    ...
    const userNavigter = useNavigate();
    ...
    const submithandler = (event) => {
        ...
        axios.post(apiBaseUrl + apiUserUrl + "/login-user", data)
            .then(
                (success) => {
                    if (success.data.status == 1) {
                        ...
                        axios.post(apiBaseUrl + apiUserUrl + "/update-user-cart/" + success.data.user._id, { state_cart: cart.data })
                            .then(
                                (success) => {
                                    if (success.data.userCart.length != 0) {
                                        ...
                                        const CartData = success.data.userCart.map(
                                            (cart) => {
                                                total += (cart.product_id.discount_price * cart.qty)
                                                return {
                                        ...
                                        dispatcher(dbToSate({ total, CartData }))
                                    }
                                }
                            ).catch(
                                (error) => {
